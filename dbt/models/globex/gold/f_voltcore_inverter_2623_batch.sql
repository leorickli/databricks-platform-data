-- Haystack-compliant gold layer for Voltcore MultiPlus-II inverter telemetry
-- Maps vendor-specific fields to Project Haystack ontology standards (inverter fields only)
-- Configuration is defined in dbt/models/schema.yml
-- Note: Battery-specific fields (state_of_charge, dc_voltage, dc_current) will be in separate battery fact table
...
-- Note: For Voltcore, device_instance 2623 maps to MultiPlus-II inverters
silver_with_keys AS (
  SELECT
    s.timestamp,
...
haystack_telemetry AS (
  SELECT
    -- Non-Haystack metadata fields
    timestamp,                      -- Measurement timestamp
    sk_inverter,                     -- Inverter device surrogate key
    sk_site,                         -- Site surrogate key
    sk_space,                        -- Space surrogate key

    -- ==================== HAYSTACK ONTOLOGY FIELDS ====================

    -- ac_power: AC Power (Watts)
    -- AC output power from inverter
    -- Ontology: lib:phScience/elec-power | Source: ac_power
    CAST(ac_power AS DECIMAL(10,2)) AS ac_power,

    -- ac_current_l1: AC Current Phase A (Amps)
    -- AC output dc_current phase 1 (single-phase inverter)
    -- Ontology: lib:phScience/elec-dc_current | Source: ac_current_l1
    CAST(ac_current_l1 AS DECIMAL(10,2)) AS ac_current_l1,

    -- ac_energy_production: Energy (Watt-hours)
    -- Rolling hourly energy production from AC output
    -- Ontology: lib:phScience/elec-energy | Source: calculated from ac_power
    CAST(energy_wh AS DECIMAL(10,2)) AS ac_energy_production,

    -- ac_voltage_l1: Phase-to-neutral Voltage Phase A (Volts)
    -- AC output dc_voltage phase 1 (single-phase inverter)
    -- Ontology: lib:phScience/elec-volt | Source: ac_voltage_l1
    CAST(ac_voltage_l1 AS DECIMAL(10,2)) AS ac_voltage_l1,

    -- ac_frequency: Line Frequency (Hertz)
    -- AC output frequency
    -- Ontology: lib:phScience/freq | Source: ac_frequency
    CAST(ac_frequency AS DECIMAL(10,2)) AS ac_frequency,

    -- temperature_cabinet: Cabinet Temperature (Celsius)
    -- Inside/cabinet temperature measurement
    -- Ontology: lib:phScience/temp | Source: temperature_cabinet
    CAST(temperature_cabinet AS DECIMAL(10,2)) AS temperature_cabinet,

    -- temperature_ambient: Ambient Temperature (Celsius)
    -- Outside/ambient temperature measurement
    -- Ontology: lib:phScience/temp | Source: temperature_ambient
    CAST(temperature_ambient AS DECIMAL(10,2)) AS temperature_ambient,

    -- operating_state: Operating State (Enum)
    -- Inverter operating state from Charge state
    -- Ontology: lib:ph/enum | Source: charge_state
    CASE
