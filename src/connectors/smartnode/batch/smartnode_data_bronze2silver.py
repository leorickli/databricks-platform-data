...
# normalized electricity metric columns:
...
# f_smartnode_pvinstallation_measurements) so each asset has its own table.
#
...
# normalized column names per Smartnode energyassetcategory:
...
def pivot_categories(
    ...
    hour with three normalized metric columns).
...
@dp.table(
    ...
    comment=(
        "Silver layer for Smartnode hourly energy bundles, normalized. One row "
    ...
    schema=StructType([
        ...
        StructField("energy_production",           DoubleType(),    True,  {"comment": "Electricity generated by PV in this hour (kWh); ENERGY OutPort, PVInstallation telemetry — from bronze category 10"}),
        StructField("energy_export",               DoubleType(),    True,  {"comment": "Electricity exported to the grid in this hour (kWh); ENERGY OutPort, EConnection telemetry — from bronze category 26"}),
        StructField("energy_import",               DoubleType(),    True,  {"comment": "Electricity imported from the grid in this hour (kWh); ENERGY InPort, EConnection telemetry — from bronze category 27"}),
