...
dbutils.widgets.text("silver_table_name", "tracksys_stream", "Silver table to monitor")
SILVER_TABLE_NAME = dbutils.widgets.get("silver_table_name")
SILVER_PATH = f"{CATALOG_NAME}.silver.{SILVER_TABLE_NAME}"

dbutils.widgets.text("state_table_name", "tracksys_ingestion_alert_state", "Source data monitor table name")
...
def send_data_gap_alert(gap_minutes, last_data_time):
    ...
    subject = f"Data Ingestion Alert: ACME/{SILVER_TABLE_NAME}"
    
    body = f"""
    Data ingestion gap detected in the ACME/TRACKSYS streaming pipeline.
    ...
    - Kinesis stream: dpx-kinesis-acme-ingestion (source)
    - Bronze layer: {CATALOG_NAME}.bronze.tracksys_stream
    - IMEI Registry: {CATALOG_NAME}.operational.imei_signal_mappings_streaming
    - Silver layer: {SILVER_PATH}
    - Kinesis outputs: dpx-kinesis-acme-bronze, dpx-kinesis-acme-silver
    
    Action Required:
    - Check source system (TRACKSYS) is sending data
    - Verify Kinesis stream connectivity
    - Check Bronze streaming job (s02_kinesis2bronze)
    - Check IMEI detection job (s03_imei_registry_stream)
    - Check Bronze to Silver job (s04_bronze2silver)
...
print(f"""
ACME/TRACKSYS Pipeline Monitoring Started:
- Target: {SILVER_PATH}
- Check Interval: {MONITORING_INTERVAL_MINUTES} minutes
- Alert Threshold: {ALERT_THRESHOLD_MINUTES} minutes
- Recipients: {', '.join(RECIPIENT_EMAILS)}

Alert Behavior:
- One email per incident (when data stops)
- Alert auto-clears when data resumes
- Ready for next incident after recovery
""")