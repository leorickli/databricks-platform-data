...
dbutils.widgets.text("silver_table_name", "kafka_inverters_stream", "Silver Source Table")
SILVER_TABLE_NAME = dbutils.widgets.get("silver_table_name")
SILVER_PATH = f"{CATALOG_NAME}.silver.{SILVER_TABLE_NAME}"

# --- Kinesis Configuration ---
dbutils.widgets.text("kinesis_stream_name", "dpx-kinesis-globex-silver", "Target Kinesis Stream Name")
KINESIS_STREAM_NAME = dbutils.widgets.get("kinesis_stream_name") or "dpx-kinesis-globex-silver"
...
CHECKPOINT_LOCATION = f"/Volumes/{CATALOG_NAME}/operational/checkpoints/kafka_inverters_silver_kinesis_sink"
...
# DBTITLE 1,Read from Silver Table
# Read the Delta table as a stream
df_silver_stream = spark.readStream.table(SILVER_PATH)
...
df_kinesis_prepared = (
    df_silver_stream
    # Use id (device id) as partitionKey to ensure ordering per device
    # If id is null, fallback to idSite or static string
    .withColumn("partitionKey",
                F.when(F.col("id").isNotNull(), F.col("id"))
                 .when(F.col("idSite").isNotNull(), F.col("idSite"))
